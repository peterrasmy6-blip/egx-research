...
def monthly_returns_from_series(points: list[dict]) -> list[float]:
    """
    Monthly returns from a dated level series.

    Takes the last observation in each calendar month, so an irregular series
    still produces a clean monthly sequence.
    """
    if not points:
        return []
    by_month: dict[str, float] = {}
    for p in points:
        d = p.get("d") or p.get("date")
        v = p.get("v") if p.get("v") is not None else p.get("close")
        if not d or v is None or v <= 0:
            continue
        by_month[str(d)[:7]] = float(v)
    keys = sorted(by_month)
    out = []
    for a, b in zip(keys, keys[1:]):
        prev, cur = by_month[a], by_month[b]
        if prev > 0:
            out.append(cur / prev - 1.0)
    return out


def _standardise(returns: list[float]) -> tuple[list[float], float, float]:
    """Turn a return series into z-scores, and report what was removed."""
    n = len(returns)
    if n < 2:
        return [], 0.0, 0.0
    logs = [math.log(1 + r) for r in returns if r > -0.99]
    m = sum(logs) / len(logs)
    var = sum((x - m) ** 2 for x in logs) / (len(logs) - 1)
    sd = math.sqrt(var) if var > 0 else 0.0
    if sd <= 0:
        return [], m, 0.0
    return [(x - m) / sd for x in logs], m, sd


def _bootstrap_path(z: list[float], months: int, rng, block: int) -> list[float]:
    """
    A sequence of standardised shocks, drawn as overlapping blocks.

    Drawing months independently destroys the two features that matter most in
    a market like Egypt's: bad months arrive in runs, and the distribution has
    far fatter tails than a bell curve. Lifting real consecutive stretches of
    history keeps both, without needing to model either.
    """
    out: list[float] = []
    top = len(z) - block
    while len(out) < months:
        start = rng.randint(0, top) if top > 0 else 0
        out.extend(z[start:start + block])
    return out[:months]
...
def monte_carlo(initial: float, monthly: float, years: int,
                ...
                mean_uncertainty: bool = True) -> dict:
    """
    ...
    # Where the *shape* of the returns comes from.
    #
    # Drawing every month independently from a bell curve produces a world with
    # no crashes: no runs of bad months, no fat tails, and above all no currency
    # break of the kind Egypt has had four times in a decade. The downside
    # percentiles are then optimistic in precisely the scenario that matters.
    #
    # So where real history is available, the shocks are lifted from it in
    # consecutive blocks, standardised, and rescaled to the return and
    # volatility the user chose. History supplies the shape; the user supplies
    # the drift. Replaying history's own drift instead would just project the
    # last few years forward, which is the error this engine already avoids
    # elsewhere.
    z_pool, method = [], "lognormal"
    if historical_monthly and len(historical_monthly) >= MIN_HISTORY_MONTHS:
        z_pool, _hm, _hsd = _standardise(historical_monthly)
        if len(z_pool) >= MIN_HISTORY_MONTHS:
            method = "block_bootstrap"

    # How wrong the expected return itself might be.
    #
    # The simulation used to treat the mean as certain and randomise only the
    # volatility around it. But the mean is the least precisely known input
    # there is -- its standard error over n years is roughly sigma/sqrt(n), so
    # with 30% volatility and ten years of data it is plus or minus about 9
    # percentage points. Ignoring that understates the spread of outcomes
    # substantially, and always in the reassuring direction.
    mu_se = 0.0
    if mean_uncertainty:
        n_years = max(1.0, (len(historical_monthly) / 12.0)
                      if historical_monthly else 10.0)
        mu_se = (annual_volatility / math.sqrt(n_years)) / 12.0
    ...
    for i in range(simulations):
        ...
        # One draw per path, not per month: the uncertainty is about which
        # world we are in, not about each month within it.
        mu_path = mu_m + (rng.gauss(0.0, mu_se) if mu_se > 0 else 0.0)
        shocks = (_bootstrap_path(z_pool, months, rng, BLOCK_MONTHS)
                  if method == "block_bootstrap" else None)
        for m in range(months):
            shock = (mu_path + sd_m * shocks[m]) if shocks is not None                 else rng.gauss(mu_path, sd_m)
    ...
    return {
        ...
        "method": method,
        "method_note": (
            "Monthly moves are lifted from real market history in consecutive "
            "six-month stretches, then rescaled to the return and volatility "
            "above. That keeps the way bad months arrive in runs and the fat "
            "tails a bell curve misses — including currency breaks of the kind "
            "Egypt has had several times."
            if method == "block_bootstrap" else
            "Monthly moves are drawn from a bell curve. Not enough market "
            "history was available to resample real ones, so runs of bad "
            "months and extreme events are under-represented."),
        "mean_uncertainty_applied": mu_se > 0,
        ...
        "assumptions": [
            ("Monthly moves are resampled from real market history in "
             "six-month stretches and rescaled to the return and volatility "
             "you chose, so runs of bad months survive."
             if method == "block_bootstrap" else
             "Monthly returns are drawn at random from a bell-shaped "
             "distribution built from the return and volatility you chose."),
            ("The expected return is itself treated as uncertain, because it "
             "is the least precisely known input there is."
             if mu_se > 0 else
             "The expected return is treated as certain."),
            "Contributions are added at the end of each month.",
        ...
        "limitations": [
            ...
            ("Resampled history covers the years we hold, which include "
             "sharp currency devaluations but not every kind of shock a market "
             "can produce. The genuine chance of a poor outcome is still "
