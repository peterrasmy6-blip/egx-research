...
def _sound():
    """
    Condition excluding bars flagged as bad prints.

    Applied in one place so no calculation can accidentally read a price the
    integrity scan has already judged wrong. The rows stay in the database --
    they are evidence of a source fault, and the coverage report counts them.
    """
    return Price.suspect.is_(None) | (Price.suspect == False)   # noqa: E712


def price_series(db, security_id: int, start: date | None = None,
                 end: date | None = None) -> list[Price]:
    q = select(Price).where(Price.security_id == security_id, _sound())
    if start:
...
def price_on_or_after(db, security_id: int, d: date) -> Price | None:
    ...
    """
    return db.scalar(
        select(Price).where(Price.security_id == security_id, _sound(), Price.d >= d)
...
def latest_price(db, security_id: int) -> Price | None:
    return db.scalar(select(Price).where(Price.security_id == security_id, _sound())
                     .order_by(Price.d.desc()).limit(1))
