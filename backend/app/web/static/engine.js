...
const ENGINE = (() => {
  ...
  const BLOCK_MONTHS = 6;
  const MIN_HISTORY_MONTHS = 48;

  /* Monthly returns from a dated level series: the last observation in each
     calendar month, so an irregular series still yields a clean sequence. */
  function monthlyReturnsFromSeries(points) {
    if (!points || !points.length) return [];
    const byMonth = {};
    for (const p of points) {
      const d = p.d || p.date;
      const v = p.v != null ? p.v : p.close;
      if (!d || v == null || v <= 0) continue;
      byMonth[String(d).slice(0, 7)] = +v;
    }
  ...
  /* Standardise a return series to z-scores so its shape can be reused with a
     different mean and volatility. */
  function standardise(returns) {
    const logs = returns.filter(r => r > -0.99).map(r => Math.log(1 + r));
    if (logs.length < 2) return [];
    const m = logs.reduce((a, b) => a + b, 0) / logs.length;
    const varr = logs.reduce((a, x) => a + (x - m) * (x - m), 0) / (logs.length - 1);
    const sd = varr > 0 ? Math.sqrt(varr) : 0;
    if (sd <= 0) return [];
    return logs.map(x => (x - m) / sd);
  }

  /* Consecutive stretches of real history rather than independent draws.
     Drawing each month separately removes the two things that matter most in a
     market like Egypt's: bad months arrive in runs, and the tails are far
     fatter than a bell curve. */
  function bootstrapPath(z, months, rand, block) {
    const out = [];
    const top = z.length - block;
    while (out.length < months) {
      const start = top > 0 ? Math.floor(rand() * (top + 1)) : 0;
      for (let k = 0; k < block && out.length < months; k++) out.push(z[start + k]);
    }
    return out;
  }
  ...
  function monteCarlo(initial, monthly, years, annualReturn, annualVolatility,
    ...
    const historicalMonthly = opts.historicalMonthly || null;
    const meanUncertainty = opts.meanUncertainty !== false;
    ...
    let zPool = [], method = "lognormal";
    if (historicalMonthly && historicalMonthly.length >= MIN_HISTORY_MONTHS) {
      zPool = standardise(historicalMonthly);
      if (zPool.length >= MIN_HISTORY_MONTHS) method = "block_bootstrap";
    }

    /* The expected return is the least precisely known input there is: its
       standard error over n years is about sigma/sqrt(n). Treating it as
       certain understates the spread of outcomes, always reassuringly. */
    let muSe = 0;
    if (meanUncertainty) {
      const nYears = Math.max(1, historicalMonthly ? historicalMonthly.length / 12 : 10);
      muSe = (annualVolatility / Math.sqrt(nYears)) / 12;
    }
    ...
    for (let i = 0; i < simulations; i++) {
      ...
      // One draw per path: the uncertainty is about which world we are in,
      // not about each month inside it.
      const muPath = muM + (muSe > 0 ? muSe * gaussian(rand) : 0);
      const shocks = method === "block_bootstrap"
        ? bootstrapPath(zPool, months, rand, BLOCK_MONTHS) : null;
      for (let m = 0; m < months; m++) {
        const shock = shocks ? muPath + sdM * shocks[m]
                             : muPath + sdM * gaussian(rand);
    ...
    return {
      type: "monte_carlo", method,
      method_note: method === "block_bootstrap"
      ...
      mean_uncertainty_applied: muSe > 0,
      simulations, years, initial, monthly,
      total_contributed: r2(contributed),
  ...
  return {
    ...
    monthlyReturnsFromSeries, standardise, bootstrapPath,
