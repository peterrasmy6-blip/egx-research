...
async function forecastPortfolioLocal(body, deps) {
  ...
  // Shape from real history, drift from the building blocks above.
  //
  // Drawing every month independently gives a world with no crashes: no runs
  // of bad months, no fat tails, and no currency break of the kind Egypt has
  // had four times in a decade. Since this page is precisely where someone
  // asks "what could go wrong", that omission matters more here than anywhere.
  let zPool = [], simMethod = "lognormal";
  try {
    const comp = await loadJSON(DATA + "/composite.json");
    const hm = (comp && comp.points)
      ? ENGINE.monthlyReturnsFromSeries(comp.points) : [];
    if (hm.length >= 48) {
      zPool = ENGINE.standardise(hm);
      if (zPool.length >= 48) simMethod = "block_bootstrap";
    }
    // The expected return is the least precisely known input; treating it as
    // certain understates the spread, always in the reassuring direction.
    var muSe = (sigma / Math.sqrt(Math.max(1, hm.length / 12))) / 12;
  } catch (e) { var muSe = 0; }
  ...
  for (let i = 0; i < sims; i++) {
    ...
    const muPath = muM + (muSe > 0 ? muSe * gauss() : 0);
    const shocks = simMethod === "block_bootstrap"
      ? ENGINE.bootstrapPath(zPool, months, rand, 6) : null;
    for (let k = 0; k < months; k++) {
      const shock = shocks ? muPath + sdM * shocks[k] : muPath + sdM * gauss();
  ...
  return {
    ...
    simulation_note: simMethod === "block_bootstrap"
      ? "Monthly moves are resampled from real Egyptian market history in "
        + "six-month stretches, so runs of bad months and the fat tails a bell "
        + "curve misses are preserved."
      : "Monthly moves are drawn from a bell curve; not enough market history "
        + "was available to resample real ones.",
