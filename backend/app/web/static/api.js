...
const {api, post} = (() => {
  ...
  // Root-absolute. With real paths, a page can now live at /stock/COMI, and a
  // relative "data/..." would resolve to /stock/data/... and 404 silently.
  const DATA = "/data";
  ...
  async function api(rawPath) {
    ...
    // The methodology page needs the same assumptions the engine ran with,
    // rather than a copy typed into the page that could drift away from them.
    if (seg[0] === "reference") return loadReference();
    if (seg[0] === "metrics") return loadMetrics();
    if (seg[0] === "quality") return loadJSON(`${DATA}/quality.json`);
    if (seg[0] === "breadth") return loadJSON(`${DATA}/breadth.json`);
    if (seg[0] === "digest") return loadJSON(`${DATA}/digest.json`);
    ...
    if (seg[0] === "screener" && seg[1] === "fields") {
      const _ref = await loadReference();
      return {fields: _ref.screener_fields,
              withheld: _ref.screener_withheld || []};
    }
    ...
    if (seg[0] === "security") {
      ...
      if (seg[2] === "fundamentals") {
        ...
        return {ticker: co.ticker, currency: co.currency, frequency: freq,
                ...
                // Only annual statements are common-sized: a quarter compared
                // against a full year's revenue would be meaningless.
                common_sized: freq === "annual"
                  ? (co.statements_common_sized || null) : null};
      }
      if (seg[2] === "dividends") {
      ...
      if (seg[2] === "valuation") {
        if (!co.valuation) {
          return {available: false,
                  history: co.valuation_history || null,
                  sensitivity: co.valuation_sensitivity || null,
        ...
        return {...co.valuation, ticker: co.ticker, name: co.name,
                as_of: co.price_date,
                // Carried alongside the model so the page can set the two
                // against each other.
                history: co.valuation_history || null,
                sensitivity: co.valuation_sensitivity || null};
      }
  ...
  async function monteCarloRequest(body) {
    ...
    // Real market history to resample the *shape* of returns from -- the runs
    // of bad months and the fat tails a bell curve cannot produce. The drift
    // still comes from the expected return above; history only supplies shape.
    let historicalMonthly = null;
    try {
      const comp = await loadJSON(`${DATA}/composite.json`);
      if (comp && comp.points)
        historicalMonthly = ENGINE.monthlyReturnsFromSeries(comp.points);
    } catch (e) { /* fall back to the bell curve, and say so in the output */ }
    ...
    const out = ENGINE.monteCarlo(body.initial, body.monthly, body.years, r, v, {
      ...
      target: body.target, historicalMonthly,
    });
    out.basis = basis;
