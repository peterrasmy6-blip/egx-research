...
class Price(Base):
    ...
    # Set when a bar is a demonstrably wrong print rather than a real session --
    # a price that leaps and returns to where it was within a few days. Such
    # bars are kept for the audit trail but excluded from every calculation.
    suspect: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
...
class SecurityMetrics(Base):
    ...
    # The same returns in constant purchasing power. Only the multi-year ones:
    # the price index is annual, so a real figure over a few months would be
    # interpolation dressed up as measurement.
    real_ret_1y: Mapped[float | None] = mapped_column(Float, nullable=True)
    real_ret_3y: Mapped[float | None] = mapped_column(Float, nullable=True)
    real_ret_5y: Mapped[float | None] = mapped_column(Float, nullable=True)

    volatility_pct: Mapped[float | None] = mapped_column(Float, nullable=True)
    max_drawdown_pct: Mapped[float | None] = mapped_column(Float, nullable=True)

    # Liquidity. On the EGX this is not a refinement -- a large share of listed
    # companies trade a few tens of thousands of pounds a day, and the most
    # common way a retail investor loses money here is not picking the wrong
    # company but buying something that cannot be sold.
    adtv_30d: Mapped[float | None] = mapped_column(Float, nullable=True)
    adtv_90d: Mapped[float | None] = mapped_column(Float, nullable=True)
    days_traded_90d: Mapped[int | None] = mapped_column(Integer, nullable=True)
    sessions_in_window: Mapped[int | None] = mapped_column(Integer, nullable=True)
    liquidity_band: Mapped[str | None] = mapped_column(String(20), nullable=True)
...
# Price rows this platform generated rather than observed as trading. They are
...
NON_TRADED_SOURCES = ("yahoo-isin-quote", "egx-research-nav",
                      ...
                      "stockanalysis-quote-intraday",
                      "african-markets-quote",
                      "african-markets-quote-intraday")


class MacroSeries(Base):
    """
    Economy-wide figures the platform needs but that are not about any one
    company -- consumer prices first among them.

    Egypt runs inflation high enough that a nominal return is close to a
    fiction: a share that rose 400% over five years while prices two-and-a-half
    times over did not make its owner four times richer. Every multi-year figure
    on this site needs a real counterpart, and that needs a price index.
    """
