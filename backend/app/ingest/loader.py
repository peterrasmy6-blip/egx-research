...
def sync_universe(verbose: bool = True, use_cache: bool = False) -> dict:
    ...
    for ticker, rec in universe.items():
        ...
        if rec.get("sector") and (not sec.sector or rec.get("sector_verified")):
            sec.sector = rec["sector"]
        db.commit()
...
def assess_coverage(verbose: bool = True) -> dict:
    ...
    for sec in db.scalars(select(Security)).all():
        ...
        if n_px == 0:
            ...
            sec.data_note = ("Prices are available, but no financial statements "
                             "were found from free sources, so profitability and "
                             "valuation measures cannot be calculated.")
        elif n_px < 250 or n_fa < 200:
            sec.data_quality = "partial"
            sec.data_note = ("Some information is unavailable from reliable free "
                             "sources. Figures shown are based on what we hold.")
        else:
